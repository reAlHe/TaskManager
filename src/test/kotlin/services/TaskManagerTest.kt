...
internal class TaskManagerTest {
    ...
    private lateinit var sut: TaskManager
    ...
    fun setUp() {
        ...
        sut = TaskManager(3, processList)
    }

    @Test
    fun `add process to task manager successfully`() {
    ...
    @Test
    fun `add process to task manager fails with MaximumProcessSizeExceededException for exceeding maximal current processes limit`() {
        val process1 = Process(UUID.randomUUID(), Priority.HIGH)
        val process2 = Process(UUID.randomUUID(), Priority.MEDIUM)
        val process3 = Process(UUID.randomUUID(), Priority.LOW)
        val process4 = Process(UUID.randomUUID(), Priority.LOW)

        sut.add(process1)
        sut.add(process2)
        sut.add(process3)

        assertThatExceptionOfType(MaximumProcessSizeExceededException::class.java).isThrownBy { sut.add(process4) }
